...
#Find minum credit per quarter that User need to take, return maxVal(largest credit from the individual class)         
def findMinCredit():
    maxVal = 0
    for i in range(len(dataList)):
...
#Draw the flow chart, show relationship by blue line (Without credit & starting quarters)
def createPlanWO():
    chartPlan.clear()
    currentQuarter = '1'
...
#It will take the classCode(CSC1230) and update the degreeCheck (prerequisits taken -> True) 
...
#It will take the classCode and update the degreeFlow (prerequisits taken -> True)      
...
#Print classes with prerequisits 
def printPlanWO():
...
#Draw the diagram of user's class schedule(With credit & starting quarters)
...
def userMenu():
    ...
    while(userOption!=6):
        ...
        if userOption == 1:  
            ...
            #if data not read, ask user to read the data first
            if fileReaded == False:
                print('---------------------------')
                ...
                #check if user put the right quarter
                quarterPass = True
                while(quarterPass):
                ...
                #Check if user put the right amount of credit (need to be larger than individual's maximum credit)
                creditPass = True
            ...
            if fileReaded == False:
                ...
                print('---------------------------')
                print('')            
            #If user did not select the quarter and credit, ask users to go back to option 2
